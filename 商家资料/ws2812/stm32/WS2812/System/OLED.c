...
void OLED_SetPos(unsigned char x, unsigned char y) //璁剧疆璧峰鐐瑰潗鏍?
{ 
...
void OLED_ShowChinese(unsigned char x, unsigned char y, unsigned char N,u8 mode)
{
	unsigned char wm=0;
	...
	for(wm = 0;wm < 16;wm++)
	{
		if(mode==1)OLED_WriteData(~(OLED_F16x16[adder]));else OLED_WriteData(OLED_F16x16[adder]);
		adder += 1;
	}
	OLED_SetPos(x,y + 1);
	for(wm = 0;wm < 16;wm++)
