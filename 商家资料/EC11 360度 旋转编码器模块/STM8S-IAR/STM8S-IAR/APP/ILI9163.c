...
void ILI9163B_write_para16(uint16 dat)
{
  ...
  buf_H = (uint8)(0xFF&(dat>>8));
  GPIOC->ODR=buf_H&0xFE; 
  GPIOE->ODR=(buf_H&0x01)<<5;
  
  GPIOD->ODR &= ~0x04;
  GPIOD->ODR |= 0x04;
 // GPIOD->ODR |= 0x10;
  
  buf_L = (uint8)(0xFF&dat);

  GPIOD->ODR &= ~0x10;
  GPIOD->ODR |= 0x04;
  
  GPIOC->ODR=buf_L&0xFE;
  GPIOE->ODR=(buf_L&0x01)<<5;
...
{
  //ILI9163B_write_command(0x2A);      	//Colulm addRSTs set   竖屏
  ILI9163B_write_command(0x2B);      	//Colulm addRSTs set   横屏
  ILI9163B_write_para8(0x00);			//行起始坐标高8位，始终为零
  ILI9163B_write_para8(xs);			//行起始坐标低8位
  ILI9163B_write_para8(0x00);			//行起始坐标高8位，始终为零
  ILI9163B_write_para8(xe);			//行终止坐标低8位
  
  //ILI9163B_write_command(0x2B);      	//Colulm addRSTs set   竖屏
  ILI9163B_write_command(0x2A);      	//Colulm addRSTs set   横屏
