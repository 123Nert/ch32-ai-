...
const unsigned char Font_code8[][6] = {                           //定义数组加const 否则数组存放到RAM中 不会存放flash中 导致编译器内存溢出错误
...
ILI9163B_SetPos(x*16,y*8,(x+1)*16-1,(y+1)*8-1);	
 for(j=0; j <16; j++)
  {
   for(i=0; i<8; i++)
    {
...
{
 uint8 i,j;
 ILI9163B_SetPos(x*8,y*6,(x+1)*8-1,(y+1)*6-1);			//定位字符显示区域
 for(j=0; j<8; j++)
  {
   for(i=0; i<6; i++)
    {
...
{
 signed char i;                  // 加上signed就不会有Warning[Pe186]: pointless comparison of unsigned integer with zero 
 uint8 ii;
 uint8 dat[6];
