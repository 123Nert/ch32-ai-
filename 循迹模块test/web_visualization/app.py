...
# 配置
MAX_DATA_POINTS = 200  # 保存最近200个数据点，和网页端显示窗口保持一致
...
def parse_serial_data(line):
    ...
    try:
        ...
        if any(keyword in upper_line for keyword in ('RAW ADC', 'AVG(4X)', 'STATUS', 'THRESHOLD', 'LINE TRACING', 'SENSOR DATA', 'ADC1_CH', '---', '===')):
            return None

        # 识别状态（BLACK 或 WHITE）
        status = 'black' if 'BLACK' in upper_line else 'white'
        ...
        # 尝试提取数字；表格格式里第二列是 Avg(4x)，优先用于绘图可降低抖动
        import re
        ...
        if numbers:
            ...
            if valid_values:
                value = valid_values[1] if '|' in line and len(valid_values) >= 2 else valid_values[0]
                return (value, status)
